...
def input_file(filename,source,destination,distance_to_add):
    ...
    for key in sorted(queue, key=queue.get):
        ...
        break
    if contents == None:
        print ("Distance:Undefined")
        print ("Route : Unrecognized")
    else:
        #print (contents)
        del queue[contents]
        ...
        if (location_to_be_sent == destination):
            ...
            # get_full_path_from_destination_to_source(reversed(location_list_to_backtrack), "Diani")
        else:
...
#input_file_name = "AICAT1_input1.txt"
input_file_name = sys.argv[1]
source_location = sys.argv[2]
#source_location = "Diani"
...
if(source_is_correct_and_destination_is_correct == True):
    ...
    print ("Distance from "+source_location+" to "+destination_location+" is Undefined")
    print ("Route = Unrecognized")